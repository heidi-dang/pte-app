          ...
          {days.map((day) => (
            <Card key={day}>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[
                  'Read Aloud practice (20 min)',
                  'Repeat Sentence drills (15 min)',
                  'Essay template review (10 min)',
                ].slice(0, Math.floor(Math.random() * 2) + 2).map((task, i) => (
                  <li key={i} style={{ fontSize: '0.875rem', color: 'var(--color-text)' }}>{task}</li>
                ))}
              </ul>
            </Card>
          ))}
