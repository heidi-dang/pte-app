...
export function filterAttemptHistory(
  ...
  if (filter.taskTypes && filter.taskTypes.length > 0) {
    filtered = filtered.filter((e) => filter.taskTypes!.includes(e.taskType));
  }
  if (filter.skills && filter.skills.length > 0) {
    filtered = filtered.filter((e) => filter.skills!.includes(e.section));
  }
  if (filter.dateRange) {
    filtered = filtered.filter(
      (e) => e.submittedAt >= filter.dateRange!.start && e.submittedAt <= filter.dateRange!.end,
    );
  }
  if (filter.mode) {
