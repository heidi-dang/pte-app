...
import { withTransaction, type DatabaseConnection, type DatabaseClient } from '@pte-app/database';
import type {
  QuestionSession,
...
export class QuestionEngineService {
  ...
  public async startSession(
    ...
    const serverDeadline = config.durationMs ? createDeadline(new Date().toISOString(), config.durationMs) : undefined;
    ...
    return withTransaction(this.connection, async (tx) => {
      ...
      // Log event
      await this.logEvent(sessionId, 'session.created', { questionId, versionId, mode }, tx);
  ...
  public async getSession(sessionId: QuestionSessionId, userId: string): Promise<QuestionSession> {
    ...
    const ownerResult = await this.connection.pool.query(`SELECT user_id FROM question_sessions WHERE id = $1`, [
      sessionId,
    ]);
  ...
  public async submitSession(
    ...
    return withTransaction(this.connection, async (tx) => {
      ...
      const submissionResult: SubmissionResult = {
        submissionId,
        sessionId,
        questionVersionId: session.questionVersionId as QuestionVersionId,
        status: 'accepted',
        idempotencyKey,
